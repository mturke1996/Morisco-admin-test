...
function useMediaQueryOld(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const [match, setMatch] = React21.useState(() => {
    if (noSsr && matchMedia) {
...
function useMediaQueryNew(query, defaultMatches, matchMedia, ssrMatchMedia, noSsr) {
  const getDefaultSnapshot = React21.useCallback(() => defaultMatches, [defaultMatches]);
  const getServerSnapshot = React21.useMemo(() => {
    if (noSsr && matchMedia) {
      return () => matchMedia(query).matches;
    }
  ...
  const [getSnapshot, subscribe] = React21.useMemo(() => {
    if (matchMedia === null) {
      return [getDefaultSnapshot, () => () => {
      }];
    }
...
function useCurrentColorScheme(options) {
  ...
  const modeStorage = React30.useMemo(() => storageManager == null ? void 0 : storageManager({
    key: modeStorageKey,
    storageWindow
  }), [storageManager, modeStorageKey, storageWindow]);
  const lightStorage = React30.useMemo(() => storageManager == null ? void 0 : storageManager({
    key: `${colorSchemeStorageKey}-light`,
    storageWindow
  }), [storageManager, colorSchemeStorageKey, storageWindow]);
  const darkStorage = React30.useMemo(() => storageManager == null ? void 0 : storageManager({
  ...
  const [isClient, setIsClient] = React30.useState(noSsr || !isMultiSchemes);
  React30.useEffect(() => {
    setIsClient(true);
  }, []);
  const colorScheme = getColorScheme(state);
  const setMode = React30.useCallback((mode) => {
  ...
  const setColorScheme = React30.useCallback((value) => {
    if (!value) {
      setState((currentState) => {
  ...
  const handleMediaQuery = React30.useCallback((event) => {
  ...
  const mediaListener = React30.useRef(handleMediaQuery);
...
function createCssVarsProvider(options) {
  ...
  const ColorSchemeContext = React31.createContext(void 0);
  if (true) {
    ColorSchemeContext.displayName = "ColorSchemeContext";
  }
  const useColorScheme = () => React31.useContext(ColorSchemeContext) || defaultContext;
  ...
  function CssVarsProvider(props) {
    ...
    const ctx = React31.useContext(ColorSchemeContext);
    const nested = !!ctx && !disableNestedContext;
    const initialTheme = React31.useMemo(() => {
      if (themeProp) {
        return themeProp;
      }
    ...
    const allColorSchemes = React31.useMemo(() => joinedColorSchemes.split(","), [joinedColorSchemes]);
    ...
    const memoTheme2 = React31.useMemo(() => {
      var _a2;
    ...
    const element = (0, import_jsx_runtime11.jsxs)(React31.Fragment, {
...
function createGrid(options = {}) {
  ...
  const Grid2 = React33.forwardRef(function Grid3(inProps, ref) {
    ...
    return (0, import_jsx_runtime13.jsx)(GridRoot, {
      ...
      children: React33.Children.map(children, (child) => {
        var _a;
        if (React33.isValidElement(child) && isMuiElement(child, ["Grid"]) && container && child.props.container) {
...
function joinChildren(children, separator) {
  const childrenArray = React34.Children.toArray(children).filter(Boolean);
  return childrenArray.reduce((output, child, index) => {
    output.push(child);
    if (index < childrenArray.length - 1) {
      output.push(React34.cloneElement(separator, {
        key: `separator-${index}`
      }));
    }
    return output;
...
function createStack(options = {}) {
  ...
  const Stack2 = React34.forwardRef(function Grid2(inProps, ref) {
    const themeProps = useThemeProps2(inProps);
    const props = extendSxProp(themeProps);
    const {
      component = "div",
...
var SvgIcon = React39.forwardRef(function SvgIcon2(inProps, ref) {
  ...
  const hasSvgAsChild = React39.isValidElement(children) && children.type === "svg";
  const ownerState = {
    ...props,
    color: color2,
    component,
...
function createSvgIcon(path, displayName) {
  ...
  return React40.memo(React40.forwardRef(Component));
}
...
@mui/private-theming/index.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-GQRJZDNI.js.map
